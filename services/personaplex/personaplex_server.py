...
def _run_command() -> Optional[str]:
    cmd = _env("PERSONAPLEX_RUN_COMMAND")
    return cmd
...
def _workdir() -> str:
    return _env("PERSONAPLEX_WORKDIR", "/var/lib/personaplex/app") or "/var/lib/personaplex/app"
...
async def chat_completions(payload: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=504,
            detail={
                "error": "personaplex subprocess timed out",
                "job_id": job_id,
