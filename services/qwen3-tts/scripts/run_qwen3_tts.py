...
def main() -> None:
    ...
    run_command = _env("QWEN3_TTS_INFER_COMMAND")
    ...
    if not run_command:
        _fail("QWEN3_TTS_INFER_COMMAND is not set")
    ...
    proc = subprocess.run(
        ...
        env=env,
        cwd=env.get("QWEN3_TTS_WORKDIR", None),
    )
    if proc.returncode != 0:
        _fail(f"QWEN3_TTS_INFER_COMMAND failed with exit code {proc.returncode}", proc.returncode)

    out_path = Path(output_path)
    if not out_path.exists():
        _fail(f"QWEN3_TTS_OUTPUT_PATH not written: {out_path}")
