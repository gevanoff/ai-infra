...
ENV_FILE="/etc/vibevoice-asr/vibevoice-asr.env"
HERE="$(cd "$(dirname "$0")" && pwd)"
SHIM_SRC="${HERE}/../vibevoice_asr_server.py"
ENV_TEMPLATE="${HERE}/../env/vibevoice-asr.env.example"
REPO_URL_DEFAULT="https://huggingface.co/microsoft/VibeVoice-ASR"
...
install_shim() {
  if [[ ! -f "$SHIM_SRC" ]]; then
    echo "ERROR: vibevoice_asr_server.py not found at ${SHIM_SRC}" >&2
    exit 1
  fi
...
clone_repo() {
  local repo_url
  repo_url="${VIBEVOICE_ASR_REPO_URL:-$REPO_URL_DEFAULT}"
  if [[ -z "$repo_url" ]]; then
    return 0
  fi
...
install_requirements() {
  ...
  if [[ -n "${VIBEVOICE_ASR_PIP_EXTRA:-}" ]]; then
    sudo -u "${SERVICE_USER}" -H "$VENV_PATH/bin/pip" install ${VIBEVOICE_ASR_PIP_EXTRA}
  fi
}
...
if [[ "$OS" == "Darwin" ]]; then
  ...
  LABEL="com.vibevoice-asr.server"
  SRC="${HERE}/../launchd/${LABEL}.plist.example"
  DST="/Library/LaunchDaemons/${LABEL}.plist"

  sudo mkdir -p "${SERVICE_HOME}" "${SERVICE_HOME}/cache" "${SERVICE_HOME}/tmp" /var/log/vibevoice-asr
  ...
  if ! id -u "${SERVICE_USER}" >/dev/null 2>&1; then
    ...
    sudo dscl . -create /Users/"${SERVICE_USER}" RealName "VibeVoice-ASR Service User"
  ...
  sudo chown -R "${SERVICE_USER}":staff "${SERVICE_HOME}" /var/log/vibevoice-asr
  sudo chmod 750 "${SERVICE_HOME}" /var/log/vibevoice-asr
  ...
  sudo -u "${SERVICE_USER}" -H "$VENV_PATH/bin/pip" install fastapi "uvicorn[standard]" httpx python-multipart
  ...
  sudo sed "s/<string>vibevoice<\/string>/<string>${SERVICE_USER}<\/string>/" "$SRC" | sudo tee "$DST" >/dev/null
...
if [[ "$OS" == "Linux" ]]; then
  if command -v systemctl >/dev/null 2>&1; then
    ...
    sudo mkdir -p "${SERVICE_HOME}" "${SERVICE_HOME}/cache" "${SERVICE_HOME}/tmp" /var/log/vibevoice-asr
    sudo chown -R "${SERVICE_USER}":"${SERVICE_USER}" "${SERVICE_HOME}" /var/log/vibevoice-asr
    sudo chmod 750 "${SERVICE_HOME}" /var/log/vibevoice-asr

    PYTHON_BIN="${VIBEVOICE_ASR_PYTHON_BIN:-}"
    ...
    SERVICE_UNIT_SRC="${HERE}/../systemd/vibevoice-asr.service"
    SERVICE_UNIT_DST="/etc/systemd/system/vibevoice-asr.service"
    sudo cp "$SERVICE_UNIT_SRC" "$SERVICE_UNIT_DST"
    sudo chmod 644 "$SERVICE_UNIT_DST"

    sudo systemctl daemon-reload
    sudo systemctl enable --now vibevoice-asr.service
    exit 0
  fi
fi
